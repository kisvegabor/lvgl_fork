...
static lv_result_t calc_cols(lv_obj_t * cont, lv_grid_calc_t * c, int32_t iteration)
{
    ...
    if(!cont->w_layout && lv_obj_get_style_width_internal(cont, 0) == LV_SIZE_CONTENT) cont_is_content = true;
...
static lv_result_t calc_rows(lv_obj_t * cont, lv_grid_calc_t * c, int32_t iteration)
{
    ...
    int32_t cont_is_content = false;
    if(!cont->h_layout && lv_obj_get_style_height_internal(cont, 0) == LV_SIZE_CONTENT) cont_is_content = true;
