...
static void flex_update_size(lv_obj_t * cont, void * user_data);
...
static void flex_update_position(lv_obj_t * cont, void * user_data)
{
    ...
    while(track_first_item < (int32_t)cont->spec_attr->child_cnt && track_first_item >= 0) {
        ...
        t.grow_dsc = NULL;
        if(rtl && !f.row) {
            *cross_pos -= track_gap + track_gap;
        }
        else {
            *cross_pos += t.track_cross_size + track_gap + track_gap;
        }
    }
...
static void children_set_grow(lv_obj_t * cont, flex_t * f, track_t * t)
{
    ...
    for(i = 0; i < t->grow_item_cnt; i++) {
        area_set_main_size(&t->grow_dsc[i].item->coords, t->grow_dsc->final_size);
    }
}
