---
title: How to Use
description: The lv_observer module is an implementation of the Observer Pattern.
---
...
| Field | Description |
|-------|-------------|
| `Subjects` | (in global memory or heap) are "logic packages", each containing the value being "observed" and its type (integer (`int32_t`), a string, a pointer, an <ApiLink name="lv_color_t" />, a `float`, or a group); |
| `Observers` | (zero or more per Subject, always dynamically-allocated) are always attached to exactly one Subject, and provide user-defined notifications each the time Subject's value changes. |
...
Using Observer first requires <ApiLink name="LV_USE_OBSERVER" /> be configured to `1`.
...
<ApiLink name="lv_subject_t" /> objects.  Reason:  their content must remain valid through
...
| Field | Description |
|-------|-------------|
| `Integer` | void <ApiLink name="lv_subject_init_int" display="lv_subject_init_int(subject, int_value)" /> |
| `Float` | void <ApiLink name="lv_subject_init_float" display="lv_subject_init_float(subject, float_value)" /> |
| `String` | void <ApiLink name="lv_subject_init_string" display="lv_subject_init_string(subject, buf, prev_buf, buf_size, initial_string)" /> |
| `Pointer` | void <ApiLink name="lv_subject_init_pointer" display="lv_subject_init_pointer(subject, ptr)" /> |
| `Color` | void <ApiLink name="lv_subject_init_color" display="lv_subject_init_color(subject, color)" /> |
| `Group` | void <ApiLink name="lv_subject_init_group" display="lv_subject_init_group(group_subject, subject_list[], count)" /> |
...
| Field | Description |
|-------|-------------|
| `Integer` | void <ApiLink name="lv_subject_set_int" display="lv_subject_set_int(subject, int_value)" /> |
| `String` | void <ApiLink name="lv_subject_copy_string" display="lv_subject_copy_string(subject, buf)" /> |
| `Pointer` | void <ApiLink name="lv_subject_set_pointer" display="lv_subject_set_pointer(subject, ptr)" /> |
| `Color` | void <ApiLink name="lv_subject_set_color" display="lv_subject_set_color(subject, color)" /> |
...
| Field | Description |
|-------|-------------|
| `Integer` | int32_t <ApiLink name="lv_subject_get_int" display="lv_subject_get_int(subject)" /> |
| `Float` | float <ApiLink name="lv_subject_get_float" display="lv_subject_get_float(subject)" /> |
| `String` | const char * <ApiLink name="lv_subject_get_string" display="lv_subject_get_string(subject)" /> |
| `Pointer` | const void * <ApiLink name="lv_subject_get_pointer" display="lv_subject_get_pointer(subject)" /> |
| `Color` | lv_color_t <ApiLink name="lv_subject_get_color" display="lv_subject_get_color(subject)" /> |
...
| Field | Description |
|-------|-------------|
| `Integer` | int32_t <ApiLink name="lv_subject_get_previous_int" display="lv_subject_get_previous_int(subject)" /> |
| `Float` | float <ApiLink name="lv_subject_get_previous_float" display="lv_subject_get_previous_float(subject)" /> |
| `String` | const char * <ApiLink name="lv_subject_get_previous_string" display="lv_subject_get_previous_string(subject)" /> |
| `Pointer` | const void * <ApiLink name="lv_subject_get_previous_pointer" display="lv_subject_get_previous_pointer(subject)" /> |
| `Color` | lv_color_t <ApiLink name="lv_subject_get_previous_color" display="lv_subject_get_previous_color(subject)" /> |
...
- <ApiLink name="lv_subject_set_min_value_int" display="lv_subject_set_min_value_int(subject, value)" />
- <ApiLink name="lv_subject_set_max_value_int" display="lv_subject_set_max_value_int(subject, value)" />
- <ApiLink name="lv_subject_set_min_value_float" display="lv_subject_set_min_value_float(subject, value)" />
- <ApiLink name="lv_subject_set_max_value_float" display="lv_subject_set_max_value_float(subject, value)" />
...
lv_observer_t * observer =  <ApiLink name="lv_subject_add_observer" display="lv_subject_add_observer(&some_subject, some_observer_cb, user_data)" />
...
<ApiLink name="lv_observer_remove" display="lv_observer_remove(observer)" /> when the Observer is no longer needed, which
...
lv_observer_t * observer =  <ApiLink name="lv_subject_add_observer_with_target" display="lv_subject_add_observer_with_target(&some_subject, some_observer_cb, some_pointer, user_data)" />

A copy of the passed pointer can be retrieved by calling
<ApiLink name="lv_observer_get_target" display="lv_observer_get_target(observer)" />, e.g. inside the callback function.
...
- If Widget does NOT need to be deleted:
    ...
    - <ApiLink name="lv_obj_remove_from_subject" display="lv_obj_remove_from_subject(widget, subject)" /> deletes all Observers
    ...
    - <ApiLink name="lv_observer_remove" display="lv_observer_remove(observer)" /> deletes that specific Observer and
...
<ApiLink name="LV_STATE_CHECKED" /> state.  When the subscribing occurs, and each time
the Subject's value is changed thereafter, the Subject's value is compared to a
reference value of `0`, and the <ApiLink name="LV_STATE_CHECKED" /> state is:
...
<ApiLink name="LV_STATE_CHECKED" /> state to be SET or CLEARED also causes the
...
<ApiLink name="lv_obj_add_subject_toggle_event" display="lv_obj_add_subject_toggle_event(obj, subject)" />
...
<ApiLink name="lv_obj_add_subject_toggle_event" display="lv_obj_add_subject_toggle_event(button1, subject1, LV_EVENT_CLICKED)" />
...
It returns a <ApiLink name="lv_subject_increment_dsc_t" /> pointer to configure the
event further:

- <ApiLink name="lv_obj_set_subject_increment_event_min_value" display="lv_obj_set_subject_increment_event_min_value(obj, dsc, min_value)" />:
  Set a minimum value for the event. Default `INT32_MIN`
- <ApiLink name="lv_obj_set_subject_increment_event_max_value" display="lv_obj_set_subject_increment_event_max_value(obj, dsc, max_value)" />:
  Set a maximum value for the event. Default `INT32_MAX`
- <ApiLink name="lv_obj_set_subject_increment_event_rollover" display="lv_obj_set_subject_increment_event_rollover(obj, dsc, rollover)" />:
...
<ApiLink name="lv_obj_add_subject_increment_event" display="lv_obj_add_subject_increment_event(button1, subject1, LV_EVENT_CLICKED, 5)" />

This will increment `subject1` by 5 when `button1` is clicked, stopping at the limits
set by the min/max values of the event or <ApiLink name="lv_subject_set_min_value_int" />
and <ApiLink name="lv_subject_set_max_value_int" /> (same for float subjects).

##### Set to a Value

- <ApiLink name="lv_obj_add_subject_set_int_event" display="lv_obj_add_subject_set_int_event(obj, subject, trigger, value)" />
- <ApiLink name="lv_obj_add_subject_set_string_event" display="lv_obj_add_subject_set_string_event(obj, subject, trigger, text)" />
